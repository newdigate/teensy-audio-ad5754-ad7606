...
audio_block_t * AudioOutputAD5754::block_input[8] = {
        nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr
};
bool AudioOutputAD5754::update_responsibility = false;
DMAChannel AudioOutputAD5754::dmatx(false);
DMAChannel AudioOutputAD5754::dmarx(false);
//DMAMEM
__attribute__((aligned(32)))
uint32_t zero_sample_buffer[AUDIO_BLOCK_SAMPLES / 2];
//DMAMEM
__attribute__((aligned(32)))
uint8_t spi_tx_buffer[32 * 4];

volatile uint16_t AudioOutputAD5754::_readIndex = 0;
volatile uint16_t AudioOutputAD5754::_writeIndex = 0;

volatile bool AudioOutputAD5754::_resetReceived = false;
audio_block_t *AudioOutputAD5754::prev[8] = {
        nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr
};
//DMAMEM
__attribute__((aligned(32)))
volatile uint8_t AudioOutputAD5754::rxbuf[32] = {0};
audio_block_t * AudioOutputAD5754::block_incoming[8] = {
        nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr
};
audio_block_t * AudioOutputAD5754::block_write[8] = {
        nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr, nullptr
};
static const uint32_t zero_level_sample = (0xFFFF / 2) + 1;
void AudioOutputAD5754::setClockDivider_noInline(uint32_t clk)
{
...
void AudioOutputAD5754::begin(void)
{
    ...
    for (uint8_t count=0; count<4; count++) {
        spi_tx_buffer[(count * 8) + 3] = 0;
        spi_tx_buffer[(count * 8) + 2] = 0;

        spi_tx_buffer[(count * 8) + 1] = count;                     //DAC0, channel=count
        spi_tx_buffer[(count * 8) + 0] = 0xFF;//(block_input[count*2] != NULL) ? block_input[count*2]->data[_readIndex] >> 8 : zero_level;
        spi_tx_buffer[(count * 8) + 7] = 0xFF;//(block_input[count*2] != NULL) ? block_input[count*2]->data[_readIndex] & 0xff : zero_level;

        spi_tx_buffer[(count * 8) + 6] = count;                     //DAC1, channel=count
        spi_tx_buffer[(count * 8) + 5] = 0xFF;//(block_input[count*2+1] != NULL) ? block_input[count*2+1]->data[_readIndex] >> 8 : zero_level;
        spi_tx_buffer[(count * 8) + 4] = 0xFF;//(block_input[count*2+1] != NULL) ? block_input[count*2+1]->data[_readIndex] & 0xff : zero_level;
        //DAC1, channel=count
    }
    ...
    NVIC_SET_PRIORITY(IRQ_LPSPI3, 23);
    NVIC_SET_PRIORITY(IRQ_DMA_CH0 + dmatx.channel, 22);
    NVIC_SET_PRIORITY(IRQ_DMA_CH0 + dmarx.channel, 21);
    NVIC_SET_PRIORITY(IRQ_GPIO6789, 20);
...
void AudioOutputAD5754::rx_complete_isr(void) {
...
void AudioOutputAD5754::update(void)
{
...
void AudioOutputAD5754::timer(void) {
    if (_readIndex < 128) {
        for (uint8_t count = 0; count < 4; count++) {
            //DAC0, channel=count
            uint8_t doubleCount = count * 2;
            audio_block_t *block = (_resetReceived) ? prev[doubleCount] : block_input[doubleCount];
            uint16_t value = (block != NULL) ? block->data[_readIndex] + zero_level_sample : zero_level_sample;
            spi_tx_buffer[(count * 8) + 0] = value >> 8;
            spi_tx_buffer[(count * 8) + 7] = value & 0xff;
            ...
            uint16_t value2 = (block2 != NULL) ? block2->data[_readIndex] + zero_level_sample : zero_level_sample;
            spi_tx_buffer[(count * 8) + 5] = value2 >> 8;
            spi_tx_buffer[(count * 8) + 4] = value2 & 0xff;
